...
# Type-only import to satisfy linters without forcing runtime dependency at dev time
if TYPE_CHECKING:  # pragma: no cover - typing only
    from sentence_transformers import SentenceTransformer as STType  # type: ignore
else:
    STType = Any  # type: ignore
...
# Compatibility route for existing clients expecting {"texts": [...]} at /model/embed
class EmbedTextsPayload(BaseModel):
    texts: List[str]


@app.post("/model/embed")
async def model_embed(payload: EmbedTextsPayload):
    if not payload.texts:
        return {"embeddings": [], "model": model_name, "dim": model_dim}
    resp = await _embed(payload.texts)
    # Match shape used elsewhere: include model + dim for completeness
    return {"embeddings": resp.embeddings, "model": resp.model, "dim": resp.dim}
...
def load_model(path: str, device: str):
    ...
    try:
        ...
        raise SystemExit(
            "Install training requirements (requirements-train.txt) to serve model."
        ) from e
    model = _ST(path, device=device)
    model_name = path
    ...
    try:
        ...
        emb = m.encode(["dimension_probe"], batch_size=1)
...
def parse_args():
    ...
    parser.add_argument(
        "--model",
        default=os.environ.get("SERVE_MODEL"),
        required=not bool(os.environ.get("SERVE_MODEL")),
    )
    ...
    parser.add_argument(
        "--device",
        default=os.environ.get("SERVE_DEVICE")
        or ("cuda" if os.environ.get("USE_CUDA", "1") == "1" else "cpu"),
    )
    parser.add_argument(
        "--concurrency", type=int, default=int(os.environ.get("SERVE_CONCURRENCY", "4"))
    )
    parser.add_argument(
        "--batch-size", type=int, default=int(os.environ.get("SERVE_BATCH_SIZE", "32"))
    )
    parser.add_argument(
        "--normalize", action="store_true", default=os.environ.get("SERVE_NORMALIZE", "0") == "1"
    )
...
def main():  # pragma: no cover - entry point
    ...
    if args.device.startswith("cuda"):
        # avoid hard dependency on torch at dev time
        import importlib
