...
_model_name = os.getenv(
    "EMBED_MODEL",
    # Prefer BGE small English by default for stronger retrieval than MiniLM
    os.getenv("EMBED_MODEL_DEFAULT", "BAAI/bge-small-en-v1.5"),
)
...
def _lazy_load():  # pragma: no cover - heavy path
    ...
    try:
        ...
        # Auto add instruction prefix if using BGE and not provided upstream.
...
async def embed_texts(payload: EmbedPayload):
    ...
    try:
        # BGE models benefit from an instruction prefix for queries; apply simple heuristic.
        texts = payload.texts
        if "bge" in _model_name.lower():
            prefix = os.getenv("BGE_PREFIX", "Represent this sentence for searching relevant passages: ")
            texts = [prefix + t if not t.lower().startswith(prefix.lower()) else t for t in texts]
