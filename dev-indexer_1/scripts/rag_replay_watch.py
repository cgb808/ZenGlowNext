...
Example:
  python scripts/rag_replay_watch.py --glob 'data/msgpack/*.msgpack' \
...
# Flexible import so it works whether run from repo root or inside scripts/
try:  # pragma: no cover - import flexibility
    from rag_replay_msgpack import replay  # type: ignore
except ImportError:  # pragma: no cover
    try:
        from scripts.rag_replay_msgpack import replay  # type: ignore
    except ImportError as e:  # pragma: no cover
        print(
            "[fatal] Could not import replay from rag_replay_msgpack; ensure PYTHONPATH includes scripts/",
            file=sys.stderr,
        )
        raise e
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Watch directory/glob for new msgpack artifacts and replay them"
    )
    ...
    ap.add_argument(
        "--sleep", type=float, default=0.0, help="Sleep between embedding micro-batches"
    )
    ap.add_argument(
        "--state-file", help="Persist processed file list to this JSON file for restart safety"
    )
    ...
    try:
        while True:
            ...
            if new_files:
                ...
                except SystemExit:  # propagate config issues
                    raise
                except Exception as e:  # noqa: BLE001
