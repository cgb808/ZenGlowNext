...
if [[ -x "${NOTIFIER_BIN}" && -f "${NOTIFIER_TPL_OPEN}" ]]; then
  count=${#file_names[@]}
  data=$(jq -n --arg tag "${BATCH_TAG}" --arg spool "${PROCESSING_DIR}" --argjson bytes ${total_bytes} --argjson count ${count} '{batch_tag:$tag, spool:$spool, bytes:$bytes, count:$count}')
...
PY
  ...
  if [[ -x "${NOTIFIER_BIN}" && -f "${NOTIFIER_TPL_DONE}" ]]; then
    ...
    data=$(jq -n --arg tag "${BATCH_TAG}" --arg status "success" --argjson processed ${processed} '{batch_tag:$tag, status:$status, processed:$processed}')
...
PY
  ...
  if [[ -x "${NOTIFIER_BIN}" && -f "${NOTIFIER_TPL_DONE}" ]]; then
    ...
    data=$(jq -n --arg tag "${BATCH_TAG}" --arg status "failed" --argjson processed ${failed} '{batch_tag:$tag, status:$status, processed:$processed}')
    "${NOTIFIER_BIN}" -template "${NOTIFIER_TPL_DONE}" -data "${data}" -v || true
  fi
fi
