...
def bulk_insert(
    conn_string: str, records: Sequence[tuple[str, str, str, str, float, str, str, str, str, float]]
):
    if not records:
        return 0
...
def main() -> None:
    ...
    parser.add_argument(
        "--max-idle-cycles",
        type=int,
        default=MAX_IDLE_CYCLES_DEFAULT,
        help="Exit after this many consecutive idle cycles (0 = run forever)",
    )
    parser.add_argument(
        "--queues", nargs="+", required=True, help="Category queue names (e.g. legal medical)"
    )
    parser.add_argument(
        "--pg-conn",
        default=os.getenv(PG_CONN_ENV, ""),
        help="Postgres connection string (or set PG_CONN_STRING env var)",
    )
    ...
    if args.max_idle_cycles > 0:
        print(
            f"→ TTL: Will exit after {args.max_idle_cycles} idle cycles ({args.max_idle_cycles * args.idle_seconds} seconds)"
        )
    ...
    while True:
        ...
        if cycle_total > 0:
            ...
            idle_cycle_count += 1
            print(
                f"(idle {args.idle_seconds}s) … [{idle_cycle_count}/{args.max_idle_cycles if args.max_idle_cycles > 0 else '∞'}]"
            )
