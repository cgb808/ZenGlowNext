...
Refactored from legacy swarm_scheduler simulation with enhancements:
 - Distinct PRIMARY vs EXPLORER swarm types.
 - least-recent queue for explorer coverage.
 - Age * quality exploitation weighting.
 - Predictive handler hook for feedback-driven quality & explorer bias adjustments.
 - Snapshot for metrics / introspection.
"""
...
class PartitionTelemetry:
    ...
    quality_ema: float = 0.1  # start slightly >0 to avoid zeroing
...
class PartitionScheduler:
    def __init__(
        ...
        # Initialize snapshot logging controls
    ...
    def _pick_explorer_partition(self) -> int:
        ...
        # fallback: novelty = low hits or low success_ema
    ...
    def route(self) -> Tuple[SwarmType, int]:
        ...
        try:
            ...
            pass
        self._t += 1
        # periodic snapshot delta logging
    ...
    def feedback(
        ...
        if self._predictive_hook:
            ...
            # Historical context if performance degrading
            ...
            try:
                ...
                pass
        # Stabilization: after any success when prob above base+0.05, gently decay toward base
...
# Convenience factory with default predictive no-op
