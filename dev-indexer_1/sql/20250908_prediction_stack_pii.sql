-- Prediction stack for PII schema: Timescale + pgvector, mirrored tables under schema `pii`
-- Safe to re-run; uses IF NOT EXISTS

BEGIN;

-- Ensure schema exists and set search path so unqualified names land in pii
CREATE SCHEMA IF NOT EXISTS pii;
SET LOCAL search_path = pii, public;

-- Extensions are database-level
DO $$
BEGIN
  BEGIN
...
-- Documents + embeddings (vector 384)
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
     WHERE table_schema='pii' AND table_name='doc_embeddings' AND column_name='embedding'
  ) THEN
