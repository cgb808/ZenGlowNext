...
-- Optional: TimescaleDB hypertable + lifecycle policies for interaction_events
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname='timescaledb') THEN
    -- Create hypertable on created_at if not already
    PERFORM create_hypertable('interaction_events','created_at', if_not_exists=>TRUE);
    -- Compression (segment by tenant, order by time)
    BEGIN
      EXECUTE 'ALTER TABLE interaction_events SET (timescaledb.compress, timescaledb.compress_segmentby = ''tenant_id'', timescaledb.compress_orderby = ''created_at DESC'')';
      PERFORM timescaledb.add_compression_policy('interaction_events', INTERVAL '14 days');
    EXCEPTION WHEN others THEN NULL; END;
    -- Retention (keep 180 days by default)
    BEGIN
      PERFORM timescaledb.add_retention_policy('interaction_events', INTERVAL '180 days');
    EXCEPTION WHEN others THEN NULL; END;
  END IF;
END$$;
