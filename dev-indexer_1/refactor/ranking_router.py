...
import psycopg  # psycopg v3 (migration from psycopg2)
...
class Query2Payload(BaseModel):
    ...
    route_hint: Optional[str] = Field(
        None,
        description="Optional route hint: 'pii' | 'specialist' | 'general' to guide DB selection",
    )
...
def _pg_connect():
    ...
    Kept lightweight (no global pool) because call volume for this router
    is modest and queries are short-lived. If this endpoint becomes hot,
    consider a global connection pool (psycopg.ConnectionPool) or reuse.
    """
...
def _pg_connect_dsn(dsn: str):
    if not dsn:
        raise HTTPException(500, "DSN not provided")
    return psycopg.connect(dsn)
...
def _similarity_search_pgvector_dsn(
    query_vec: List[float], k: int, dsn: str
) -> List[Candidate]:
    sql = """
        SELECT id, chunk, embedding <-> %s::vector AS dist, source
        FROM doc_embeddings
        ...
        """
    try:
        with _pg_connect_dsn(dsn) as conn:
            with conn.cursor() as cur:
                cur.execute(sql, (query_vec, query_vec, k))
                rows = cur.fetchall()
    except Exception as e:
        logging.getLogger(__name__).warning(
            "retrieval_failed_secondary", extra={"event": "retrieval", "error": str(e)}
        )
        return []
    out: List[Candidate] = []
    for r in rows:
...
async def rag_query2(payload: Query2Payload) -> Dict[str, Any]:
    ...
    route_hint = (payload.route_hint or "").lower().strip() or None
    ...
    if retrieval_mode == "disabled":
        ...
        elif retrieval_mode in ("pgvector_multi", "multi", "pg_multi"):
            # Gate multi-source retrieval behind env toggle
            if os.getenv("DB_MULTI_ENABLED", "0") != "1":
                candidates = _similarity_search_pgvector(q_vec, top_k)
            else:
                # Query primary and secondary Postgres and merge to top_k by distance
                secondary_dsn = os.getenv("SECONDARY_DATABASE_URL") or os.getenv(
                    "SUPABASE_DB_URL"
                )
                primary_k = (top_k + 1) // 2
                secondary_k = top_k - primary_k
                primary = _similarity_search_pgvector(q_vec, primary_k)
                secondary = (
                    _similarity_search_pgvector_dsn(q_vec, secondary_k, secondary_dsn)
                    if secondary_dsn
                    else []
                )
                # Dedupe by text content, prefer lower distance
                merged: List[Candidate] = []
                seen_text: Dict[str, float] = {}
                for cand in primary + secondary:
                    key = cand.text
                    if key in seen_text:
                        if cand.distance < seen_text[key]:
                            # replace worse existing
                            for i, m in enumerate(merged):
                                if m.text == key:
                                    merged[i] = cand
                                    seen_text[key] = cand.distance
                                    break
                    else:
                        merged.append(cand)
                        seen_text[key] = cand.distance
                    if len(merged) >= top_k:
                        # keep filling until we have k unique texts
                        pass
                # If one side was sparse, top up from the other with remaining results
                if len(merged) < top_k:
                    pool = (primary + secondary)
                    # sort by distance and include new ones until k
                    pool_sorted = sorted(pool, key=lambda c: c.distance)
                    for cand in pool_sorted:
                        if cand.text in seen_text:
                            continue
                        merged.append(cand)
                        seen_text[cand.text] = cand.distance
                        if len(merged) >= top_k:
                            break
                candidates = merged[:top_k]
        elif retrieval_mode in ("pgvector_route", "route", "pg_route"):
            # Gate route-based DSN selection behind env toggle
            if os.getenv("DB_ROUTE_ENABLED", "0") != "1":
                candidates = _similarity_search_pgvector(q_vec, top_k)
            else:
                # Choose DSN by route_hint with fallbacks
                dsn = None
                if route_hint == "pii":
                    dsn = os.getenv("PII_DATABASE_URL") or os.getenv("DATABASE_URL")
                elif route_hint == "specialist":
                    dsn = os.getenv("SPECIALIST_DATABASE_URL") or os.getenv("SECONDARY_DATABASE_URL") or os.getenv("SUPABASE_DB_URL") or os.getenv("DATABASE_URL")
                else:  # general/default
                    dsn = os.getenv("GENERAL_DATABASE_URL") or os.getenv("DATABASE_URL")

                try:
                    candidates = _similarity_search_pgvector_dsn(q_vec, top_k, dsn) if dsn else []
                except Exception:
                    candidates = _similarity_search_pgvector(q_vec, top_k)
        else:
            candidates = _similarity_search_pgvector(q_vec, top_k)
