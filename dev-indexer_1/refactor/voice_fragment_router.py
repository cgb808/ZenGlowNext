...
import os
from app.central_control.session_state import append_fragment as mem_append_fragment, get_session as mem_get_session
from app.core.redis_async import get_async_redis
try:
    from app.central_control.session_state_redis import append_fragment as redis_append_fragment, get_session as redis_get_session
except Exception:  # pragma: no cover
    redis_append_fragment = None  # type: ignore
    redis_get_session = None  # type: ignore
...
async def ingest_fragment(payload: VoiceFragment) -> VoiceRouteResponse:
    use_redis = os.getenv("VOICE_SESSION_BACKEND", "memory").lower() in {"redis", "auto"}
    redis_client = None
    if use_redis and redis_get_session and redis_append_fragment:
        redis_client = get_async_redis()
        # If auto but redis not available, fallback seamlessly
        if not redis_client and os.getenv("VOICE_SESSION_BACKEND", "memory").lower() == "auto":
            use_redis = False

    if use_redis and redis_client:
        snap_before = await redis_get_session(redis_client, payload.session_id)  # type: ignore[arg-type]
    else:
        snap_before = mem_get_session(payload.session_id)
    before_tokens = _token_count(snap_before['accum_text'])
    if use_redis and redis_client:
        updated = await redis_append_fragment(redis_client, payload.session_id, payload.fragment, payload.is_final)  # type: ignore[arg-type]
    else:
        updated = mem_append_fragment(payload.session_id, payload.fragment, payload.is_final)
