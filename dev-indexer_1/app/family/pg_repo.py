...
class PgFamilyRepo:
    ...
    # Stubs below; flesh out as needed
...
class PgFamilyRepo:
    ...
    # Relationships ----------------------------------------------------
    def add_guardian(
        self, guardian_id: str, child_id: str, legal: bool = True, app_user: Optional[str] = None
    ) -> None:
        sql = """
    ...
    def list_artifacts(
        self, limit: int = 100, app_user: Optional[str] = None
    ) -> list[dict[str, Any]]:
        conn_ctx = self._conn_with_user(app_user) if app_user else self._conn()
        with conn_ctx as c, c.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:  # type: ignore
            cur.execute(
                "SELECT * FROM family_artifacts ORDER BY created_ts DESC LIMIT %s", (limit,)
            )
            return list(cur.fetchall())
    ...
    def latest_health_metric(
        self, entity_id: str, metric: str, app_user: Optional[str] = None
