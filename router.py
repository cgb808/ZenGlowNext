...
from .context import ensure_seed, Person
...
def list_people(svc: BaseFamilyService = Depends(get_family_service), user: str = Depends(get_family_user)):
    return [PersonOut(**p) for p in svc.list_people(user_id=user) if p]

@router.post("/people", response_model=PersonOut)
def upsert_person(body: PersonIn, svc: BaseFamilyService = Depends(get_family_service), user: str = Depends(get_family_user)):
    p = svc.upsert_person({"id": body.id, "name": body.name, "age": body.age, "roles": body.roles or [], "meta": body.meta or {}}, user_id=user)
    return PersonOut(**p)

@router.get("/people/{pid}", response_model=PersonOut)
def get_person(pid: str, svc: BaseFamilyService = Depends(get_family_service), user: str = Depends(get_family_user)):
...
def add_guardian(guardian_id: str, child_id: str, svc: BaseFamilyService = Depends(get_family_service), user: str = Depends(get_family_user)):
    try:
...
def tutoring_guardrails(child_id: str, svc: BaseFamilyService = Depends(get_family_service), user: str = Depends(get_family_user)):
...
def add_health_metric(entity_id: str, body: HealthMetricIn, svc: BaseFamilyService = Depends(get_family_service), user: str = Depends(get_family_user)):
...
def add_bucket_item(entity_id: str, bucket_type: str, body: BucketItemIn, svc: BaseFamilyService = Depends(get_family_service), user: str = Depends(get_family_user)):
...
def list_bucket_items(entity_id: str, bucket_type: str, limit: int = 50, svc: BaseFamilyService = Depends(get_family_service), user: str = Depends(get_family_user)):
...
def add_artifact(body: ArtifactIn, svc: BaseFamilyService = Depends(get_family_service), user: str = Depends(get_family_user)):
    return svc.add_artifact(body.entity_id, body.kind, body.title, body.tags, body.meta, body.content_ref)

@router.get("/artifacts")
def list_artifacts(entity_id: str | None = None, tag: str | None = None, kind: str | None = None, limit: int = 100, svc: BaseFamilyService = Depends(get_family_service), user: str = Depends(get_family_user)):
